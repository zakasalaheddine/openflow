...
export function systemPrompt(input: { brandProfile: string; ops: Ops }) {
  ...
  return [
    ...
    '- An image into another image is a reference too: that rendered still is sent to the model as a reference for the new one. This is how a character stays the same person — build a sheet of them first, then wire it into every shot. It counts against the same reference budget as a source does.',
    '- An image into a video is that clip\'s first frame. A clip may have exactly one.',
    '- Only a video may feed a sequence. Wire the clips in the order they should play; the person can reorder them afterwards.',
    '- A sequence feeds nothing further — the film it cuts is the last step. The person downloads it themselves.',
